...
func forceRebalance(ctx context.Context, client k8sclient.Client, knn KindNameSpaceName, pods []string) error {
	...
	err := wait.Poll(time.Second*5, time.Second*5, func() (done bool, err error) {
		if knn.Kind.GetObjectKind().GroupVersionKind().Kind == "DeploymentConfig" {
...
func updatePodBalanceAttemptsOnKNN(ctx context.Context, knn KindNameSpaceName, client k8sclient.Client) error {
	obj := knn.Kind
	...
	if err != nil {
		return fmt.Errorf("Error getting dc %s on namespace %s. %w", knn.Name, knn.Namespace, err)
	}
	metaObj, err := meta.Accessor(obj)
	if err != nil {
		return err
	}
	ant, err := getAnnotations(metaObj.GetAnnotations(), knn.Name, ns)
	if err != nil {
		return err
	}
	dc.SetAnnotations(ant)
	if err := client.Update(ctx, obj); err != nil {
		return fmt.Errorf("Error Updating dc %s on %s. %w", knn.Name, knn.Namespace, err)
	}
	logrus.Infof("Successfully updated dc %s on %s", knn.Name, knn.Namespace)
	return nil
}
